...
def run_experiment(run, seed, T, delta, resultDir):
    random.seed(seed)
...
def main():
    ...
    resultDir = '../data/hardtolearn/S=' + str(nState) + ', T=' + str(T) + '/UCRL2'
    ...
    results = pool.starmap(run_experiment, [(run, seeds[run], T, delta, resultDir) for run in range(runs)])
    ...
    plt.title("Hard To Learn: nState=6, T=10000")
