...
from env.env import *
from algorithms.lsvi_dc_hard import LSVI_DC
import multiprocessing as mp
import matplotlib.pyplot as plt
import seaborn as sns

# Function to run a single experiment
def run_experiment(run, seed, T, nState, H, delta, gamma, lam, resultDir):
    random.seed(seed)
    ...
    agent = LSVI_DC(env, T=T, H=H, delta=delta, gamma=gamma, lam=lam)
    # Run the agent
...
def main():
    ...
    resultDir = '../data/hardtolearn/S=' + str(nState) + ', T=' + str(T) + '/LSVI-DC(linear_mixture)'
    ...
    results = pool.starmap(run_experiment, [(run, seeds[run], T, nState, H, delta, gamma, lam, resultDir) for run in range(runs)])
