...
from env.env import *
from algorithms.optimal_policy_hard import Optimal_Policy
import multiprocessing as mp
import matplotlib.pyplot as plt
import seaborn as sns

# Function to run a single experiment
def run_experiment(run, seed, T, nState, resultDir):
    random.seed(seed)
...
def main():
    ...
    resultDir = '../data/hardtolearn/S=' + str(nState) + ', T=' + str(T) + '/Optimal Policy'
    ...
    results = pool.starmap(run_experiment, [(run, seeds[run], T, nState, resultDir) for run in range(runs)])
