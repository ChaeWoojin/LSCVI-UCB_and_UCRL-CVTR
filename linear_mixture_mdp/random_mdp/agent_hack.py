...
class UCRL_CVTR_hack:
    def __init__(self, env, init_s, gamma, phi, lambda_reg, B, H, theta_star):
        '''
    ...
    def execute_policy(self, Q, V, Sigma_k, t_k, N_k, pbar):
        ...
        while not self.terminate_episode(Sigma_k):
            s_t = self.env.state
            xi_t, a_t = self.select_action_for_state(s_t, Q, N_k)
    ...
    def compute_Q(self, s, a, V, theta_k, beta_k, Sigma_k):
        ...
        probabilities = np.dot(self.phi[s, a, :], self.theta_star)
